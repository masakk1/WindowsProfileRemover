...
function SurveyUsers() {
    ...
    foreach ($User in $AllUsers) {
        # En C:\Users\
	    If ($User.LocalPath -notlike "$UsersFolder*") {PrintUserExclusion -User $User -Reason "usuario fuera de $UsersFolder"; Continue}
        # No es local
...
function EliminarYReportarUsuarios() {
    ...
    Write-Progress -Activity "Eliminando Perfiles" -Status "Terminado de borrar perfiles" -PercentComplete 100
